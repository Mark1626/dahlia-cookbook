...
int main()
{
  ...
  // inputDistances is the array of distances and
  // outputDistance is the result.
  tsp(inputDistances, outputDistance);

  auto outputDistanceUInt32 = (unsigned int) outputDistance[0];
  // Print result
  std::cout << "Shortest trip (raw): " << outputDistance[0] << "\n";
  // std::cout << "Shortest trip: " << (float)(outputDistanceUInt16 * maxDist) / (float)(maxUint) << "\n";

  // Check against expected answer only for N from 1 to 11...

  if (outputDistance[0] == expected[N-1]) {
    std::cout << "Result matches...\n";
    return 0;
    }
  else {
    std::cout << "Result " << outputDistance[0] << " does not match expected result !!! -> " << expected[N-1] << "\n";
    return 1;
  }
}